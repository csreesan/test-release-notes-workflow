# just trick to get a space
_empty :=
_space := $(_empty) $(empty)

# Gers added after version
# ?= means set only if it is not already set
VERSION_POST ?=o
...
DEFAULT_BUMP ?= minor
# else bump batch by default
else
DEFAULT_BUMP ?= patch
endif

# git rev-parse is an ancillary plumbing command primarily used for manipulation
# one common use to for printing SHA1 hashes given a revision specifier 
# e.g. --short for printing out shorter unique SHA1
# --git-dir for displaying the abs/relative path of the .git directory
# --is-inside-work-tree checking if you're currently within a work tree, --is-inside-git-dir is for if in git repo
# git describe finds the most recent tag that is reachable from a commit...
VERSION ?= $(shell git rev-parse --is-inside-work-tree > /dev/null && git describe --tags --always --dirty)
ifneq (,$(findstring dirgy, $(VERSION)))
...
# tr is used to delete or substitue, e.g. a='a\nb' echo $a | tr '\n' ' ' -> a b if instad  tr '\n' -> ab
GIT_MESSAGES := $(shell git rev-parse --is-inside-worktree > /dev/null && git log --pretty='%s' v$(CLEAN_VERSION_)...HEAD 2>/dev/null | tr '\n' ' ')
...
# $(subst a,b,c) is the substitute command replaces a with b in c
...
tag-release:
	...
	# || true is used to prevent stop the shell from exiting as soon as a command exits with a non-zero status
	git tag -d $(BUMPED_VERSION) || true
	git push $(GIT_REMOTE_NAME) :$(BUMPED_VERSION) || true
	git push tag $(BUMPED_VERSION)
...
commit-release:
	# --exit codde makes git diff exxit with 1 if there were differnces and 0 if no differences
	# --cached view changes you staged for the next commit
	# --name-status show only name and status of changed files
